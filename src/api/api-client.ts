...
export default class ApiClient {
    ...
    public constructor() {
        ...
        this.axios_instance.interceptors.response.use(async config => {
            ...
            if (
                [423, 429, 403, 409].some(error_status_code => error_status_code === config.status) &&
                retries
            ) {
                retries--;

                const timeout = config.headers['retry-after']
                    ? parseInt(config.headers['retry-after'], 10) * 1000
                    : 5000;
                ...
                logger.debug(
                    `Received status: ${config.status}. Retrying after ${timeout}. Attempts left: ${retries}`,
                );
                await sleep(timeout);
